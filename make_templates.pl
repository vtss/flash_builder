...
sub do_image {
    ...
    } elsif ($layout eq "linux-nor") {
        return if ($fsize < SZ_32M);
        my ($used) = sum(map { $_->{size} } @entries) + $tail_overhead;
        my ($imgsize) = SZ_1M*(13.5); # Istax target limit - 32MB boards
        my ($datasize) = block_round_down($fsize - $used - (2*$imgsize), $bsize);
        my ($imgfile) =
            push @entries, (
                {
                    'name'     => 'linux',
...
my (@boards) = (
    ...
    {
        name       => "serval1",
        geometries => [ [SZ_8M, SZ_64K], [SZ_16M, SZ_64K], [SZ_16M, SZ_256K], [SZ_32M, SZ_64K] ],
    ...
    },
    {
        ...
        geometries => [ [SZ_8M, SZ_64K], [SZ_16M, SZ_64K], [SZ_32M, SZ_64K] ],
    ...
    {
        name       => "ocelot_8-cu4sfp8",
        geometries => [ [SZ_8M, SZ_64K], [SZ_8M, SZ_64K], [SZ_16M, SZ_256K], [SZ_32M, SZ_64K] ],
        redboot    => "artifacts/redboot-ocelot.img",
        linux      => "artifacts/bringup_ocelot_8.mfi",
    },
    {
        name       => "ocelot_10-cu4sfp8",
        geometries => [ [SZ_8M, SZ_64K], [SZ_8M, SZ_64K], [SZ_16M, SZ_256K], [SZ_32M, SZ_64K] ],
        redboot    => "artifacts/redboot-ocelot.img",
        linux      => "artifacts/bringup_ocelot_10.mfi",
    },
    {
        name       => "serval2",
        geometries => [ [SZ_8M, SZ_64K], [SZ_16M, SZ_64K], [SZ_32M, SZ_64K] ],
...
@types = qw(linux linux-nor) unless(@types);
